  ...
  return (
    <section>
      <div className="flex items-center gap-3 mb-4">
        ...
        <h2 className="text-lg font-semibold text-neutral">Top Qarzdorlar</h2>
        <div className="ml-auto flex items-center gap-2">
          <span className="text-xs text-base-content/60">Ko'rsatish:</span>
          {[10, 20, 50].map(n => (
            <button
              key={n}
      ...
      <div className="card bg-base-100 shadow-sm border border-base-200 overflow-hidden">
          ...
          <table className="table table-sm">
            <thead className="bg-base-200 text-base-content/70">
              <tr>
                ...
                <th>Telefon</th>
                <th
                  className="cursor-pointer hover:text-primary select-none text-right"
            ...
            <tbody>
              {isLoading
                ...
                : sorted.map((item, idx) => {
                    ...
                    const debtLevel =
                      item.debt_amount > 5_000_000 ? 'text-error font-bold' :
                      item.debt_amount > 2_000_000 ? 'text-warning font-semibold' :
                      'text-base-content'
                    return (
                      <tr key={idx} className="hover:bg-base-200/50">
                        <td className="text-base-content/50 font-mono text-xs">{idx + 1}</td>
                        <td>
                          <span className={`badge badge-sm ${t.cls}`}>{t.label}</span>
                        </td>
                        ...
                        <td className={`text-right tabular-nums ${debtLevel}`}>
                          {fmtSum(item.debt_amount)}
                        </td>
                      </tr>
                    )
                  })
        ...
        {results.length > 0 && (
          <div className="px-4 py-2 border-t border-base-200 text-xs text-base-content/50 flex justify-between">
